...
public class InventoryHandler {
	...
	public static void ItemGather(LittleEndianAccessor slea, MapleClient c) {
		...
		List<Item> itemMap = Inv.list().stream().filter(item -> item.getPosition() <= 128)
	...
	public static boolean UseRewardBox(final byte slot, final int itemId, final MapleClient c,
		...
		if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId && !player.hasBlockedInventory()) {
			if (player.getInventory(MapleInventoryType.EQUIP).getNextFreeSlot() > -1
					&& player.getInventory(MapleInventoryType.USE).getNextFreeSlot() > -1
					&& player.getInventory(MapleInventoryType.SETUP).getNextFreeSlot() > -1
					&& player.getInventory(MapleInventoryType.ETC).getNextFreeSlot() > -1) {
				...
				if (list != null && list.size() > 0) {
					...
					MapleInventoryManipulator.addById(c, rewardItemId, (short) quantity,
							"打开随机箱子 道具ID: " + itemId + " 时间: " + DateUtil.getNowTime());
					c.announce(MaplePacketCreator.getShowItemGain(rewardItemId, (short) 1, true));
					MapleInventoryManipulator.removeFromSlot(c, ItemConstants.getInventoryType(itemId), slot, (short) 1,
							false);
					return true;
				}
			...
			if (chr.getInventory(MapleInventoryType.EQUIP).getNextFreeSlot() > -1
				...
				if (itemId == 2028048) { // 未知金币包
					...
					if (mesars > 0 && chr.getMeso() < (Integer.MAX_VALUE - mesars)) {
						...
						MapleInventoryManipulator.removeFromSlot(c, ItemConstants.getInventoryType(itemId), slot,
								(short) 1, false);
						return true;
					} else {
						...
						for (Map<String, String> reward : rewards.getRight()) {
							...
							if (prob > 0 && Randomizer.nextInt(rewards.getLeft()) < prob) { // Total prob
								if (ItemConstants.getInventoryType(rewardItemId) == MapleInventoryType.EQUIP) {
									...
									MapleInventoryManipulator.addById(c, rewardItemId, quantity,
											"Reward item: " + itemId + " on " + DateUtil.getCurrentDate());
									c.announce(MaplePacketCreator.getShowItemGain(rewardItemId, quantity, true));
								}
								// MapleInventoryManipulator.removeById(c,
								// GameConstants.getInventoryType(itemId), itemId, 1, false, false);
								MapleInventoryManipulator.removeFromSlot(c, ItemConstants.getInventoryType(itemId),
										slot, (short) 1, false);
								c.announce(EffectPacket.showRewardItemAnimation(rewardItemId, effect));
								chr.getMap().broadcastMessage(chr,
										EffectPacket.showRewardItemAnimation(rewardItemId, effect, chr.getId()), false);
								return true;
							}
						}
					}
	...
	public static void UseItem(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		if (chr == null || !chr.isAlive() || chr.getMapId() == 749040100 || chr.getMap() == null
				|| chr.hasDisease(MapleDisease.无法使用药水) || chr.hasBlockedInventory() || chr.inPVP()
				|| chr.getMap().isPvpMaps()) {
			c.announce(MaplePacketCreator.enableActions());
			return;
		}
	...
	public static void UseMiracleCube(LittleEndianAccessor slea, MapleCharacter chr) {
		...
		if (bl2) {
			...
			MapleInventoryManipulator.removeFromSlot(chr.getClient(), MapleInventoryType.USE, scrollSlot, (short) 1,
					false, true);
		}
	...
	public static void UseMagnify(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		...
		if (chr.isShowPacket()) {
			chr.dropMessage(5, "鉴定装备: 放大镜: " + magnify + " insight: " + insight + " toScroll: " + toScroll
					+ " BlockedInventory: " + c.getPlayer().hasBlockedInventory());
		}
		...
		if (((toScroll.getState(false) < 17 && toScroll.getState(false) > 0)
				|| (toScroll.getState(true) < 17 && toScroll.getState(true) > 0))
				&& (insight || magnify.getItemId() == 2460005 || magnify.getItemId() == 2460004
						|| magnify.getItemId() == 2460003 || (magnify.getItemId() == 2460002 && n3 <= 12)
						|| (magnify.getItemId() == 2460001 && n3 <= 7)
						|| (magnify.getItemId() == 2460000 && n3 <= 3))) {
			...
			if (ItemConstants.isZeroWeapon(nEquip.getItemId())) {
				...
				chr.forceUpdateItem(((Equip) chr.getInventory(MapleInventoryType.EQUIPPED).getItem((short) dst))
						.copyPotential(nEquip), true);
			}
			chr.getTrait(MapleTraitType.insight).addExp((insight ? 10 : ((magnify.getItemId() + 2) - 2460000)) * 2,
					chr);
			chr.getMap().broadcastMessage(
					InventoryPacket.showMagnifyingEffect(chr.getId(), toScroll.getPosition(), !isPotAdd));
			if (!insight) {
	...
	public static void UseSpReset(byte slot, int itemId, MapleClient c, MapleCharacter chr) {
		Item toUse = chr.getInventory(ItemConstants.getInventoryType(itemId)).getItem(slot);
		if (toUse == null || itemId / 1000 != 2500 || toUse.getItemId() != itemId
				|| JobConstants.is新手职业(chr.getJob())) {
			c.announce(MaplePacketCreator.enableActions());
			return;
		}
		chr.spReset();
			...
			if (!MapleItemInformationProvider.getInstance().isMobHP(itemid) || mob.getHp() <= mob.getMobMaxHp() / 2) {
				...
				if (MapleItemInformationProvider.getInstance().getCreateId(itemid) > 0) {
					MapleInventoryManipulator.addById(c, MapleItemInformationProvider.getInstance().getCreateId(itemid),
							(short) 1, "Catch item " + itemid + " on " + DateUtil.getCurrentDate());
				}
			} else {
		...
		if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId && !chr.hasBlockedInventory()
			...
			case 2430007: { // Blank Compass
				...
				if (inventory.countById(3994102) >= 20 // 罗盘用N
					...
					MapleInventoryManipulator.addById(c, 2430008, (short) 1,
							"Scripted item: " + itemId + " on " + DateUtil.getCurrentDate()); // Gold Compass
					...
					MapleInventoryManipulator.addById(c, 2430007, (short) 1,
							"Scripted item: " + itemId + " on " + DateUtil.getCurrentDate()); // Blank Compass
				}
					...
					if (c.getPlayer().getInventory(MapleInventoryType.USE).countById(2430112) >= 25) {
						if (MapleInventoryManipulator.checkSpace(c, 2049400, 1, "") && MapleInventoryManipulator
							...
							MapleInventoryManipulator.addById(c, 2049400, (short) 1,
									"Scripted item: " + toUse.getItemId() + " on " + DateUtil.getCurrentDate());
						} else {
							c.getPlayer().dropMessage(5, "消耗栏空间位置不足.");
						}
					} else if (c.getPlayer().getInventory(MapleInventoryType.USE).countById(2430112) >= 10) {
						if (MapleInventoryManipulator.checkSpace(c, 2049401, 1, "") && MapleInventoryManipulator
						...
						// c.getPlayer().dropMessage(5, "There needs to be 10 Fragments for a Potential
						// Scroll, 25 for Advanced Potential Scroll.");
					}
				} else {
					c.getPlayer().dropMessage(5, "消耗栏空间位置不足.");
				}
				...
				if (c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
					if (c.getPlayer().getInventory(MapleInventoryType.USE).countById(2430481) >= 100) {
						if (MapleInventoryManipulator.checkSpace(c, 2049701, 1, "") && MapleInventoryManipulator
								.removeById(c, MapleInventoryType.USE, toUse.getItemId(), 100, true, false)) {
			...
			case 2430760: // 星岩魔方碎片
				if (c.getPlayer().getInventory(MapleInventoryType.CASH).getNumFreeSlot() >= 1) {
					if (c.getPlayer().getInventory(MapleInventoryType.USE).countById(2430760) >= 10) {
						if (MapleInventoryManipulator.checkSpace(c, 5750000, 1, "") && MapleInventoryManipulator
								.removeById(c, MapleInventoryType.USE, toUse.getItemId(), 10, true, false)) {
							MapleInventoryManipulator.addById(c, 5750000, (short) 1,
									"Scripted item: " + toUse.getItemId() + " on " + DateUtil.getCurrentDate());
						} else {
						...
						if (MapleInventoryManipulator.checkSpace(c, newId, 1, "") && MapleInventoryManipulator
							...
							if (grade == 2) { // [B]级星岩
								...
								WorldBroadcastService.getInstance()
										.broadcastMessage(MaplePacketCreator.getGachaponMega(c.getPlayer().getName(),
												" : 从星岩箱子中获得{" + ii.getName(newId) + "}！大家一起恭喜他（她）吧！！！！", nItem,
												(byte) 1, c.getChannel()));
							} else if (grade == 3) {
								Item nItem = new Item(newId, (byte) 0, (short) 1, (byte) 0);
								WorldBroadcastService.getInstance()
							...
							chr.getMap().broadcastMessage(
									InventoryPacket.showNebuliteEffect(chr.getId(), true, "成功交换了星岩。"));
			...
			case 2430144: // 秘密能手册
				...
				if (MapleItemInformationProvider.getInstance().itemExists(itemid)
					...
					MapleInventoryManipulator.addById(c, itemid, (short) 1,
							"Reward item: " + toUse.getItemId() + " on " + DateUtil.getCurrentDate());
					MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (byte) 1, false);
				}
				break;
			case 2430370: // 秘密配方
			...
			case 2430142: // 卡珊德拉的专属补给品箱
				if (c.getPlayer().getInventory(MapleInventoryType.EQUIP).getNumFreeSlot() >= 1) {
					if (c.getPlayer().getJob() == 3200 || c.getPlayer().getJob() == 3210
							|| c.getPlayer().getJob() == 3211 || c.getPlayer().getJob() == 3212) {
						MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (byte) 1, false);
						MapleInventoryManipulator.addById(c, 1382101, (short) 1,
								"Scripted item: " + itemId + " on " + DateUtil.getCurrentDate());
					} else if (c.getPlayer().getJob() == 3300 || c.getPlayer().getJob() == 3310
							|| c.getPlayer().getJob() == 3311 || c.getPlayer().getJob() == 3312) {
						MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (byte) 1, false);
						MapleInventoryManipulator.addById(c, 1462093, (short) 1,
								"Scripted item: " + itemId + " on " + DateUtil.getCurrentDate());
					} else if (c.getPlayer().getJob() == 3500 || c.getPlayer().getJob() == 3510
							|| c.getPlayer().getJob() == 3511 || c.getPlayer().getJob() == 3512) {
						MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (byte) 1, false);
...
//                    break;
			...
			default:
				if (ItemConstants.isDamageSkinItem(toUse.getItemId())) {
					...
					if (chr.getQuestNAdd(MapleQuest.getInstance(7291)).getCustomData() == null
							|| !chr.getQuestNAdd(MapleQuest.getInstance(7291)).getCustomData().equals(damageskin)) {
						...
						MapleInventoryManipulator.removeFromSlot(c,
								ii.isCash(toUse.getItemId()) ? MapleInventoryType.CASH : MapleInventoryType.USE, slot,
								(short) 1, false);
					} else {
		...
		if (mountid > 0) {
			...
			// System.err.println("骑宠技能 - 3 " + mountid + " 是否有技能: " +
			// (SkillFactory.getSkill(mountid) == null) + " 骑宠: " +
			// (GameConstants.getMountItem(mountid, c.getPlayer()) == 0));
			if (c.getPlayer().getSkillLevel(mountid) > 0) {
				c.getPlayer().dropMessage(1,
						"您已经拥有了[" + SkillFactory.getSkill(mountid).getName() + "]这个骑宠的技能，无法使用该道具。");
				...
				c.getPlayer().dropMessage(1,
						"恭喜您获得[" + SkillFactory.getSkill(mountid).getName() + "]骑宠技能 " + expiration_days + " 权。");
	...
	public static void UseTreasureChest(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		/*
		 * [B4 00] [19 00] [C0 4E 41 00] [01] - 没有永恒的热度提示使用点卷 800 最后面的就是 1 [B4 00] [19
		 * 00] [C0 4E 41 00] [00]
		 */
		...
		if (useCash && chr.getCSPoints(2) < price) {
			...
			if (rareness > 0) {
				WorldBroadcastService.getInstance()
						.broadcastMessage(MaplePacketCreator.getGachaponMega(c.getPlayer().getName(),
								" : 从" + box + "中获得{" + ii.getName(item.getItemId()) + "}！大家一起恭喜他（她）吧！！！！", item,
								rareness, c.getChannel()));
			}
		} else {
	...
	public static void Pickup_Player(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		...
		try {
			...
			if (Distance > 5000 && (mapitem.getMeso() > 0 || mapitem.getItemId() != 4001025)) {
				...
				// chr.getCheatTracker().registerOffense(CheatingOffense.ITEMVAC_CLIENT,
				// String.valueOf(Distance));
				WorldBroadcastService.getInstance().broadcastGMMessage(
						MaplePacketCreator.serverNotice(6, "[GM消息] " + chr.getName() + " ID: " + chr.getId() + " (等级 "
								+ chr.getLevel() + ") 全屏捡物。地图ID: " + chr.getMapId() + " 范围: " + Distance));
	...
	public static void Pickup_Pet(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		...
		try {
			...
			if (Distance > 10000 && (mapitem.getMeso() > 0 || mapitem.getItemId() != 4001025)) {
				...
				// chr.getCheatTracker().registerOffense(CheatingOffense.PET_ITEMVAC_CLIENT,
				// String.valueOf(Distance));
...
//                WorldBroadcastService.getInstance().broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM消息] " + chr.getName() + " ID: " + chr.getId() + " (等级 " + chr.getLevel() + ") 全屏宠吸。地图ID: " + chr.getMapId() + " 范围: " + Distance));
	...
	private static void removeItem(MapleCharacter chr, MapleMapItem mapitem, MapleMapObject ob) {
		...
		mapitem.setPickedUp(true);
		chr.getMap().broadcastMessage(InventoryPacket.removeItemFromMap(mapitem.getObjectId(), 2, chr.getId()),
				mapitem.getPosition());
...
//            int type = slea.readByte(); //未知 应该是搜索类型
	...
	public static void PamSong(LittleEndianAccessor slea, MapleClient c) {
		...
		if (slea.readByte() > 0 && c.getPlayer().getScrolledPosition() != 0 && pam != null && pam.getQuantity() > 0) {
			MapleInventoryType inv = c.getPlayer().getScrolledPosition() < 0 ? MapleInventoryType.EQUIPPED
	...
	public static void UseNebulite(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		...
		chr.getMap().broadcastMessage(
				InventoryPacket.showNebuliteEffect(c.getPlayer().getId(), success, success ? "成功嵌入星岩。" : "嵌入星岩失败。"));
		c.announce(MaplePacketCreator.enableActions());
	}
	...
	public static void UseAdditionalAddItem(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		...
		if (scroll == null || scroll.getQuantity() < 0 || !ItemConstants.isSealAddItem(scroll.getItemId())
				|| toScroll == null || toScroll.getQuantity() != 1) {
			c.announce(MaplePacketCreator.enableActions());
			return;
		}
		MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
		...
		if (successRate <= 0) {
			c.getPlayer().dropMessage(1, "卷轴道具: " + scroll.getItemId() + " - " + ii.getName(scroll.getItemId())
					+ " 成功几率为: " + successRate + " 该卷轴可能还未修复.");
			c.announce(MaplePacketCreator.enableActions());
			return;
		}
		if (chr.isAdmin()) {
			chr.dropSpouseMessage(0x0B, "卷轴道具: " + scroll.getItemId() + " - " + ii.getName(scroll.getItemId())
					+ " 成功几率为: " + successRate + "% 卷轴是否失败不消失装备: " + noCursed);
		}
		if (toScroll.getPotential(1, true) == 0 || toScroll.getPotential(2, true) == 0
				|| toScroll.getPotential(3, true) == 0) {
			boolean success = false;
			...
			if (Randomizer.nextInt(100) <= successRate) {
				...
				while (!rewarded) {
					...
					if (option != null && !GameConstants.isAboveA(option.opID) && option.reqLevel / 10 <= rank
							&& GameConstants.optionTypeFits(option.optionType, toScroll.getItemId())
							&& GameConstants.potentialIDFits(option.opID, 17, 1)
							&& GameConstants.isBlockedPotential(toScroll, option.opID, true, false)) {
			...
			/*
			 * 如果没有成功 2048200 - 低级潜能附加印章 - 成功几率 5 % 失败 100 %消失道具 2048201 - 中级潜能附加印章 - 成功几率 5
			 * % 失败 100 %消失道具
			 */
	...
	public static void UseAdditionalItem(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		...
		final Equip toScroll = (Equip) chr
				.getInventory((toSlot < 0) ? MapleInventoryType.EQUIPPED : MapleInventoryType.EQUIP).getItem(toSlot);
		if (scroll == null || scroll.getQuantity() < 0 || !ItemConstants.isSealItem(scroll.getItemId())
				|| toScroll == null || toScroll.getQuantity() != 1) {
			c.sendEnableActions();
			return;
		}
		...
		if (chr.isAdmin()) {
			chr.dropSpouseMessage(11, "卷轴道具: " + scroll.getItemId() + " - " + ii.getName(scroll.getItemId())
					+ " 成功几率为: " + successRate + "%");
		}
		if (toScroll.getPotential(1, false) == 0 || toScroll.getPotential(2, false) == 0
				|| toScroll.getPotential(3, false) == 0) {
			boolean success = false;
			final int lines = (toScroll.getPotential(2, false) == 0) ? 2
					: ((toScroll.getPotential(1, false) == 0) ? 1 : 3);
			...
			if (Randomizer.nextInt(100) <= successRate) {
				...
				while (!rewarded) {
					...
					if (option != null && !GameConstants.isAboveA(option.opID) && option.reqLevel / 10 <= rank
							&& GameConstants.optionTypeFits(option.optionType, toScroll.getItemId())
							&& GameConstants.potentialIDFits(option.opID, 17, 1)
							&& GameConstants.isBlockedPotential(toScroll, option.opID, false, false)) {
				...
				if (ItemConstants.isZeroWeapon(toScroll.getItemId())) {
					chr.forceUpdateItem(((Equip) chr.getInventory(MapleInventoryType.EQUIPPED)
							.getItem((short) ((toSlot == -10) ? -11 : -10))).copyPotential(toScroll), true);
				}
				success = true;
			}
			toScroll.initAllState();
		...
		if (shop != null && itemId == shop.getItemId() && shop.getTokenPrice() > 0) {
			if (chr.getInventory(MapleInventoryType.ETC).countById(4310029) >= shop.getTokenPrice()) {
				if (MapleInventoryManipulator.checkSpace(c, shop.getItemId(), 1, "")) {
					MapleInventoryManipulator.removeById(c, MapleInventoryType.ETC, 4310029, shop.getTokenPrice(), true,
							false);
					MapleInventoryManipulator.addById(c, shop.getItemId(), (short) 1, shop.getPotentialGrade(),
							"十字商店购买: " + DateUtil.getCurrentDate());
	...
	public static void UseSoulScroll(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		...
		chr.getInventory(ItemConstants.getInventoryType(scroll.getItemId())).removeItem(scroll.getPosition(), (short) 1,
				false);
		List<ModifyInventory> mods = new ArrayList<>();
		...
		if (Randomizer.nextInt(100) < success) {
			...
			chr.getMap().broadcastMessage(chr, MaplePacketCreator.showSoulScrollEffect(chr.getId(), false, false),
					true);
		}
		c.announce(InventoryPacket.modifyInventory(true, mods, chr));
	}
	...
	public static void applyHyunCube(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		...
		if (chr.getOneInfo(52998, "dst") != null) {
			...
			final Equip equip = (Equip) chr
					.getInventory((intValue > 0) ? MapleInventoryType.EQUIP : MapleInventoryType.EQUIPPED)
					.getItem((short) intValue);
			if (equip != null) {
	...
	public static void UseFamiliarCard(MapleCharacter chr) {
		...
		cards.forEach(integer -> {
			...
			int grade = Randomizer.nextInt(1000) < 50 ? 3
					: (50 <= random && random < 300 ? 2 : (300 <= random && random < 750 ? 1 : 0));
