...
public class MapleMapFactory {
	...
	private static final MapleDataProvider source = MapleDataProviderFactory
			.getDataProvider(new File(System.getProperty("wzpath") + "/Map.wz"));
	private static final MapleData nameData = MapleDataProviderFactory
			.getDataProvider(new File(System.getProperty("wzpath") + "/String.wz")).getData("Map.img");
					...
					for (MapleData data : mapleData) {
						for (MapleData subdata : data) {
							if (subdata.getName().equalsIgnoreCase("mapName")) {
								jedis.hset(RedisUtil.KEYNAMES.MAP_NAME.getKeyName(), data.getName(),
										subdata.getData().toString());
							}
						}
					}
				}
					...
					for (MapleDataDirectoryEntry directoryEntry1 : directoryEntry.getSubdirectories()) {
						if (directoryEntry1.getName().startsWith("Map")) {
							for (MapleDataFileEntry fileEntry : directoryEntry1.getFiles()) {
								for (MapleData life : source.getData(directoryEntry.getName() + "/"
										+ directoryEntry1.getName() + "/" + fileEntry.getName())) {
									if (life.getName().equals("life")) {
										...
										for (MapleData mapleData : life) {
											...
											if (type != null && type.equals("n")) {
												npcids.add(
														MapleDataTool.getIntConvert(mapleData.getChildByPath("id"), 0));
											}
										}
										if (!npcids.isEmpty()) {
											try {
												jedis.hset(RedisUtil.KEYNAMES.MAP_LINKNPC.getKeyName(),
														Integer.valueOf(fileEntry.getName().substring(0, 9)).toString(),
														JsonUtil.getMapperInstance().writeValueAsString(npcids));
	...
	public static Map<Integer, List<Integer>> getAllLinkNpc() {
		...
		for (Map.Entry<String, String> entry : tempdata.entrySet()) {
			try {
				ret.put(Integer.valueOf(entry.getKey()),
						JsonUtil.getMapperInstance().readValue(entry.getValue(), new TypeReference<List<Integer>>() {
						}));
			} catch (IOException e) {
				e.printStackTrace();
			}
			...
			try {
				...
				try (DruidPooledConnection con = DatabaseConnection.getInstance().getConnection()) {
					...
					while (rs.next()) {
						...
						AbstractLoadedMapleLife sqlmyLife = loadLife(sqlid, sqlf, sqlhide, sqlfh, sqlcy, sqlrx0, sqlrx1,
								sqlx, sqly, sqltype, mapid);
						switch (sqltype) {
						case "n":
				...
				/*
				 * load life data (npc, monsters) 加载地图刷新的NPC和怪物信息
				 */
				...
				map.setCreateMobInterval((short) MapleDataTool.getInt(mapData.getChildByPath("info/createMobInterval"),
						ServerConfig.CHANNEL_MONSTER_REFRESH * 1000));
				...
				try {
					map.setMapName(MapleDataTool.getString("mapName",
							nameData.getChildByPath(getMapStringName(linkMapId > 0 ? linkMapId : omapid)), ""));
	...
	public MapleMap CreateInstanceMap(int mapid, boolean respawns, boolean npcs, boolean reactors, int instanceid) {
		...
		try (DruidPooledConnection con = DatabaseConnection.getInstance().getConnection()) {
			...
			while (rs.next()) {
				...
				int sqly = rs.getInt("y");
				int sqlmobTime = rs.getInt("mobtime");
				AbstractLoadedMapleLife sqlmyLife = loadLife(sqlid, sqlf, sqlhide, sqlfh, sqlcy, sqlrx0, sqlrx1, sqlx,
						sqly, sqltype, mapid);
				if (sqltype.equals("n")) {
					map.addMapObject(sqlmyLife);
	...
	private AbstractLoadedMapleLife loadLife(MapleData life, String id, String type, int mapid) {
		...
		myLife.setPosition(new Point(MapleDataTool.getIntConvert(life.getChildByPath("x")),
				MapleDataTool.getIntConvert(life.getChildByPath("y"))));
	...
	private AbstractLoadedMapleLife loadLife(int id, int f, boolean hide, int fh, int cy, int rx0, int rx1, int x,
			int y, String type, int mapid) {
		...
		if (mapData.getChildByPath("nodeInfo") != null) {
			for (MapleData node : mapData.getChildByPath("nodeInfo")) {
				try {
					...
					MapleNodeInfo mni = new MapleNodeInfo(Integer.parseInt(node.getName()),
			...
			if (mapData.getChildByPath(String.valueOf(i)) != null && mapData.getChildByPath(i + "/obj") != null) {
				for (MapleData node : mapData.getChildByPath(i + "/obj")) {
					if (node.getChildByPath("SN_count") != null && node.getChildByPath("speed") != null) {
						...
						MaplePlatform mni = new MaplePlatform(name, MapleDataTool.getIntConvert("start", node, 2),
