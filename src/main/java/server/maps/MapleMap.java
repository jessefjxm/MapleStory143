...
public final class MapleMap {
	...
	private boolean town, clock, personalShop, miniMapOnOff, everlast = false, dropsDisabled = false,
			gDropsDisabled = false, soaring = false, squadTimer = false, isSpawns = true, checkStates = true;
	...
	public void addAreaMonsterSpawn(MapleMonster monster, Point pos1, Point pos2, Point pos3, int mobTime, String msg,
			boolean shouldSpawn, boolean sendWorldMsg) {
...
//        }
		...
		if (hasClock()) {
			...
			chr.send((MaplePacketCreator.getClockTime(cal.get(Calendar.HOUR_OF_DAY), cal.get(Calendar.MINUTE),
		...
		if (getNumMonsters() > 0
				&& (mapid == 280030001 || mapid == 240060201 || mapid == 280030000 || mapid == 280030100
		...
		if ((mapid == 10000 && chr.getJob() == 0) || (mapid == 130030000 && chr.getJob() == 1000)
				|| (mapid == 914000000 && chr.getJob() == 2000) || (mapid == 900010000 && chr.getJob() == 2001)
				|| (mapid == 931000000 && chr.getJob() == 3000)) {
			chr.send(MaplePacketCreator
					.startMapEffect("欢迎来到 " + chr.getClient().getChannelServer().getServerName() + "!", 5122000, true));
		...
		chr.send(MaplePacketCreator.showChronosphere(chr.getBossLog("免费超时空券"),
				(int) Math.ceil(chr.getCSPoints(2) / 200)));
	}
	 ...
	 * 计算方法暂时如下.
	 *
	 * @试运行: 一天24小时 1小时：41点 1小时等于60分钟 60分钟划分为6份 每份10分钟 每10分钟1次 每次7点 @按等级计算公式： int
	 *       givNx = ((jsNx * chr.getLevel()/2) + Nx); @备注信息： 以上计算方法结果如下：
	 *       100级玩家/每天最多获得1000点券。
	 */
...
//                }
	...
	/*
	 * public void broadcastMessage(MapleCharacter source, byte[] packet, boolean
	 * repeatToSource, boolean ranged) { broadcastMessage(repeatToSource ? null :
	 * source, packet, ranged ? MapleCharacter.MAX_VIEW_RANGE_SQ :
	...
	public boolean canSpawn(long now) {
		return lastSpawnTime > 0 && lastSpawnTime
				+ (ServerConfig.CHANNEL_MONSTER_REFRESH > 0 ? ServerConfig.CHANNEL_MONSTER_REFRESH * 1000
						: createMobInterval) < now;
	}
	...
	public void checkMoveMonster(Point from, boolean fly, MapleCharacter chr) {
		...
		// System.err.println("检测数量: " + check + " 怪物数量: " + getMonstersInRange(from,
		// 4000.0).size() + " 最大刷新: " + maxRegularSpawn);
	...
	public void checkStates(String chr) {
		...
		// if (squad != null) {
		// final MapleSquad sqdd =
		// ChannelServer.getInstance(channel).getMapleSquad(squad);
		// if (sqdd != null && chr != null && chr.length() > 0 &&
		// sqdd.getAllNextPlayer().contains(chr)) {
		// sqdd.getAllNextPlayer().remove(chr);
		// broadcastMessage(MaplePacketCreator.serverNotice(5, "The queued player " +
		// chr + " has left the map."));
		// }
		// }
	}
	...
	public void disappearingItemDrop(MapleMapObject dropper, MapleCharacter owner, Item item, Point pos) {
		...
		broadcastMessage(InventoryPacket.dropItemFromMapObject(drop, dropper.getTruePosition(), droppos, (byte) 3),
	...
	public void doShrine(final boolean spawned) { // false = entering map, true = defeated
		...
		final int mode = ((mapid == 280030000 || mapid == 280030100) ? 1
...
//                            }
	...
	private void dropFromMonster(MapleCharacter chr, MapleMonster mob, boolean instanced) {
		if (mob == null || chr == null || ChannelServer.getInstance(channel) == null || dropsDisabled
				|| mob.dropsDisabled() || chr.getPyramidSubway() != null || ServerConfig.WORLD_BANDROPITEM) { // no
																												// drops
																												// in
																												// pyramid
																												// ok?
...
//        int cashServerRate = ChannelServer.getInstance(channel).getCashRate(); //点卷爆率
		...
		for (MonsterDropEntry de : dropEntry) {
			...
			if (Randomizer.nextInt(999999) < (int) (de.chance * dropServerRate * chr.getDropMod()
					* (chr.getStat().getDropBuff() / 100.0) * (挑衅加成 / 100.0))) {
				if (mesoDropped && droptype != 3 && de.itemId == 0) { // not more than 1 sack of meso
					continue;
				}
				if (de.itemId / 10000 == 238) { // 去掉怪物宝怪物卡 && !mob.getStats().isBoss() &&
				...
				if (de.itemId == 0) { // meso
					...
					if (mesos > 0) {
						spawnMobMesoDrop(
								(int) (mesos * (chr.getStat().mesoBuff / 100.0) * chr.getDropMod() * mesoServerRate),
					...
					if (ItemConstants.isNoticeItem(de.itemId)) {
						broadcastMessage(MaplePacketCreator.serverNotice(6,
								"[掉宝提示] 玩家 " + chr.getName() + " 在 " + chr.getMap().getMapName() + " 杀死 "
										+ mob.getStats().getName() + " 掉落道具 " + ii.getName(de.itemId)));
					}
		...
		for (MonsterGlobalDropEntry de : globalEntry) {
			...
			if (Randomizer.nextInt(999999) < de.chance * globalServerRate
				...
				if (!gDropsDisabled) {
					...
					idrop.setGMLog(
							"怪物掉落: " + mob.getId() + " 地图: " + mapid + " (Global) 时间: " + DateUtil.getCurrentDate());
					if (ItemConstants.isNoticeItem(de.itemId)) {
						broadcastMessage(MaplePacketCreator.serverNotice(6,
								"[掉宝提示] 玩家 " + chr.getName() + " 在 " + chr.getMap().getMapName() + " 杀死 "
										+ mob.getStats().getName() + " 掉落道具 " + ii.getName(de.itemId)));
...
//        }
		...
		} else if (n2 / 100000 == 93 && player.getMapId() / 1000000 == 955
				&& this.getAllMonstersThreadsafe().isEmpty()) {
			switch (player.getMapId() % 1000 / 100) {
			case 1:
			case 2: {
		...
		} else if (n2 / 100000 == 93 && (player.getMapId() == 921160200 || player.getMapId() == 921160400)
				&& this.getAllMonster().isEmpty()) {
			this.startMapEffect("请快点移动到下一张地图。", 5120053);
		} else if (n2 / 100000 == 93 && player.getMapId() / 10000 == 24008 && this.getAllMonster().isEmpty()
				&& eim != null) {
	...
	public List<MapleMapObject> getAllDoorsThreadsafe() {
		...
		try {
			ret = mapobjects.get(MapleMapObjectType.DOOR).values().parallelStream()
					.filter(mmo -> mmo instanceof MapleDoor).collect(Collectors.toCollection(ArrayList::new));
		} finally {
	...
	public List<MapleMapObject> getAllMechDoorsThreadsafe() {
		...
		try {
			ret = mapobjects.get(MapleMapObjectType.DOOR).values().parallelStream()
					.filter(mmo -> mmo instanceof MechDoor).collect(Collectors.toCollection(ArrayList::new));
		} finally {
			mapobjectlocks.get(MapleMapObjectType.DOOR).readLock().unlock();
		}
		return ret;
	...
	public List<MapleMonster> getAllMonstersThreadsafe(boolean filter) {
		...
		try {
			ret = mapobjects.get(MapleMapObjectType.MONSTER).values().parallelStream()
					.filter(mmo -> filter || !((MapleMonster) mmo).getStats().getName().contains("dummy")
							&& !((MapleMonster) mmo).getStats().isFriendly())
					.map(mmo -> (MapleMonster) mmo).collect(Collectors.toCollection(ArrayList::new));
//            ret = mapobjects.get(MapleMapObjectType.MONSTER).values().parallelStream()
...
//                    .collect(Collectors.toCollection(ArrayList::new));
	...
	public List<MapleReactor> getAllReactorsThreadsafe() {
		...
		mapobjectlocks.get(MapleMapObjectType.REACTOR).readLock().lock();
		try {
			ret = mapobjects.get(MapleMapObjectType.REACTOR).values().parallelStream().map(mmo -> (MapleReactor) mmo)
					.collect(Collectors.toCollection(ArrayList::new));
		} finally {
			mapobjectlocks.get(MapleMapObjectType.REACTOR).readLock().unlock();
		}
	...
	public List<MapleRune> getAllRuneThreadsafe() {
		...
		try {
			ret = mapobjects.get(MapleMapObjectType.RUNE).values().parallelStream().map(mmo -> (MapleRune) mmo)
					.collect(Collectors.toCollection(ArrayList::new));
		} finally {
	...
	public List<MapleSummon> getAllSummonsThreadsafe() {
		...
		try {
			ret = mapobjects.get(MapleMapObjectType.SUMMON).values().parallelStream()
					.filter(mmo -> mmo instanceof MapleSummon).map(mmo -> (MapleSummon) mmo)
					.collect(Collectors.toCollection(ArrayList::new));
		} finally {
	...
	public List<MapleMapObject> getMapObjectsInRange(Point from, double rangeSq) {
		...
		for (MapleMapObjectType type : MapleMapObjectType.values()) {
			mapobjectlocks.get(type).readLock().lock();
			try {
				mapobjects.get(type).values().stream().filter(mmo -> from.distanceSq(mmo.getTruePosition()) <= rangeSq)
						.forEach(ret::add);
			} finally {
				mapobjectlocks.get(type).readLock().unlock();
			}
	...
	public List<MapleMapObject> getMapObjectsInRect(Rectangle box, List<MapleMapObjectType> MapObject_types) {
		...
		for (MapleMapObjectType type : MapObject_types) {
			mapobjectlocks.get(type).readLock().lock();
			try {
				mapobjects.get(type).values().stream().filter(mmo -> box.contains(mmo.getTruePosition()))
						.forEach(ret::add);
			} finally {
				mapobjectlocks.get(type).readLock().unlock();
			}
	...
	public int getNumMonsters(int mobid) {
		...
		try {
			if (mobid == -1) {
				...
				return (int) mapobjects.get(MapleMapObjectType.MONSTER).entrySet().stream()
						.filter(entry -> ((MapleMonster) entry.getValue()).getId() == mobid).count();
			}
		} finally {
			mapobjectlocks.get(MapleMapObjectType.MONSTER).readLock().unlock();
		}
	...
	public List<MapleCharacter> getPlayersInRectAndInList(Rectangle box, List<MapleCharacter> chrList) {
		...
		charactersLock.readLock().lock();
		try {
			character = characters.stream().filter(a -> chrList.contains(a) && box.contains(a.getTruePosition()))
	...
	public List<MaplePortal> getPortalsInRange(Point from, double rangeSq) {
		return portals
				.values().stream().filter(type -> from.distanceSq(type.getPosition()) <= rangeSq
						&& type.getTargetMapId() != mapid && type.getTargetMapId() != 999999999)
				.collect(Collectors.toList());
	}

	public List<MaplePortal> getPortalSP() {
		return portals.values().stream().filter(port -> port.getName().equals("sp"))
	...
	public void getRankAndAdd(String leader, String time, ExpeditionType type, long timz, Collection<String> squad) {
		try {
			...
			if (lastTime == 0) { // great, we just add it
				SpeedRunner.addSpeedRunData(type, SpeedRunner.addSpeedRunData(
						new StringBuilder(SpeedRunner.getPreamble(type)), new HashMap<>(), z, leader, 1, time), timz);
			} else {
	...
	public void killMonster(MapleMonster monster) { // For mobs with removeAfter
		...
		broadcastMessage(MobPacket.killMonster(monster.getObjectId(),
				monster.getStats().getSelfD() < 0 ? 1 : monster.getStats().getSelfD()));
		removeMapObject(monster);
		monster.killed();
	}

	public void killMonster(MapleMonster monster, MapleCharacter chr, boolean withDrops, boolean second,
			byte animation) {
	...
	public void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops,
		...
		/*
		 * 8810122 - 进阶暗黑龙王 8810018 - 暗黑龙王的灵魂
		 */
				...
				for (MapleCharacter mc : characters) {
					if (mc.isAlive()) {
						...
						case 8820212: // 混沌品克缤 buffid = 2022449
							mc.getClient().announce(EffectPacket.showOwnBuffEffect(buffid,
									EffectOpcode.UserEffect_JobChanged.getValue(), mc.getLevel(), 1)); // HT nine spirit
							broadcastMessage(mc, EffectPacket.showBuffeffect(mc, buffid,
									EffectOpcode.UserEffect_JobChanged.getValue(), mc.getLevel(), 1), false); // HT nine
																												// spirit
							break;
						}
					}
		...
		if (mobid == 8810018 && mapid == 240060200) { // 暗黑龙王
			WorldBroadcastService.getInstance()
					.broadcastMessage(MaplePacketCreator.serverNotice(6, "经过无数次的挑战，终于击破了暗黑龙王的远征队！你们才是龙之林的真正英雄~"));
			charactersLock.readLock().lock();
			try {
			...
			WorldBroadcastService.getInstance()
					.broadcastMessage(MaplePacketCreator.serverNotice(6, "经过无数次的挑战，终于击破了进阶暗黑龙王的远征队！你们才是龙之林的真正英雄~"));
			charactersLock.readLock().lock();
			try {
				for (MapleCharacter c : characters) {
			...
			WorldBroadcastService.getInstance()
					.broadcastMessage(MaplePacketCreator.serverNotice(6, "凭借永不疲倦的热情打败品克缤的远征队啊！你们是真正的时间的胜者！"));
			charactersLock.readLock().lock();
			try {
				for (MapleCharacter c : characters) {
			...
			WorldBroadcastService.getInstance()
					.broadcastMessage(MaplePacketCreator.serverNotice(6, "凭借永不疲倦的热情打败混沌品克缤的远征队啊！你们是真正的时间的胜者！"));
			charactersLock.readLock().lock();
			try {
				for (MapleCharacter c : characters) {
			...
			doShrine(true);
		} else if ((mobid == 8850011 && mapid == 271040200) || (mobid == 8850012 && mapid == 271040100)) { // 希纳斯
																											// 274040200
			WorldBroadcastService.getInstance()
					.broadcastMessage(MaplePacketCreator.serverNotice(6, "被黑魔法师黑化的希纳斯女皇终于被永不言败的远征队打倒! 混沌世界得以净化!"));
			charactersLock.readLock().lock();
			try {
				for (MapleCharacter c : characters) {
			...
			for (MapleMapObject mmo : getAllMonstersThreadsafe()) {
				...
				if (mons.getLinkOid() != monster.getObjectId()) {
					...
					// FileoutputUtil.log(FileoutputUtil.Pinkbean_Log, "地图杀死怪物 8820008 : " +
					// mons.getId() + " - " + mons.getStats().getName(), true);
				}
			}
			...
			for (MapleMapObject mmo : getAllMonstersThreadsafe()) {
				...
				if (mons.getId() != 8820000 && mons.getId() != 8820001 && mons.getObjectId() != monster.getObjectId()
					...
					killMonster(mons, chr, false, false, animation);
					// FileoutputUtil.log(FileoutputUtil.Pinkbean_Log, "地图杀死怪物 品克缤 : " +
					// mons.getId() + " - " + mons.getStats().getName(), true);
				}
			}
			...
			for (MapleMapObject mmo : getAllMonstersThreadsafe()) {
				...
				if (mons.getLinkOid() != monster.getObjectId()) {
					killMonster(mons, chr, false, false, animation);
					// FileoutputUtil.log(FileoutputUtil.Pinkbean_Log, "地图杀死怪物 8820108 : " +
					// mons.getId() + " - " + mons.getStats().getName(), true);
				}
			}
			...
			for (MapleMapObject mmo : getAllMonstersThreadsafe()) {
				MapleMonster mons = (MapleMonster) mmo;
				if (mons.getId() != 8820100 && mons.getId() != 8820212 && mons.getObjectId() != monster.getObjectId()
						&& mons.isAlive() && mons.getLinkOid() == monster.getObjectId()) {
					killMonster(mons, chr, false, false, animation);
					// FileoutputUtil.log(FileoutputUtil.Pinkbean_Log, "地图杀死怪物 混沌品克缤 : " +
					// mons.getId() + " - " + mons.getStats().getName(), true);
				}
			}
		...
		if (type != null) {
			if (speedRunStart > 0 && speedRunLeader.length() > 0) {
				...
				broadcastMessage(MaplePacketCreator.serverNotice(5,
						speedRunLeader + "带领的远征队，耗时: " + time + " 击败了 " + name + "!"));
		...
		try {
			...
			while (itr.hasNext()) {
				...
				if (!npc.isCustom() || npcid != -1 && npc.getId() != npcid
						|| npc.getId() != 0 && npc.getOwnerid() != ownerid)
					continue;
	...
	public void respawn(boolean force, long now) {
		...
		if (force) { // cpq quick hack
			...
			int defaultNum = (GameConstants.isForceRespawn(mapid) ? monsterSpawn.size() : maxRegularSpawn)
					- spawnedMonstersOnMap.get();
			int numShouldSpawn = ServerConfig.CHANNEL_MONSTER_MAXCOUNT > 0
					? Math.max(defaultNum, ServerConfig.CHANNEL_MONSTER_MAXCOUNT)
					: defaultNum;
			if (numShouldSpawn > 0) {
				int spawned = 0;
				...
				for (Spawns spawnPoint : randomSpawn) {
					...
					if (spawnPoint.shouldSpawn(lastSpawnTime) || GameConstants.isForceRespawn(mapid)
							|| (monsterSpawn.size() < 10 && maxRegularSpawn > monsterSpawn.size()
									&& partyBonusRate > 0)) {
						spawnPoint.spawnMonster(this);
						spawned++;
					}
	...
	private void respawnReactor(MapleReactor reactor) {
		if (!isSecretMap() && reactor.getReactorId() >= 100000 && reactor.getReactorId() <= 200011) {
			...
			// System.err.println("重置反应堆 - toSpawnPos: " + toSpawnPos.size() + " herbRocks:
			// " + spawnPoints.size());
			MapleReactor newReactor = new MapleReactor(MapleReactorFactory.getReactor(newRid), newRid);
			newReactor.setPosition(toSpawnPos.isEmpty() ? reactor.getPosition()
					: toSpawnPos.get(Randomizer.nextInt(toSpawnPos.size())));
			newReactor.setDelay(newRid % 100 == 11 ? 60000 : 5000);
			spawnReactor(newReactor);
			// System.err.println("重置反应堆 - oldId: " + reactor.getReactorId() + " newId: " +
			// newReactor.getReactorId() + " 是否相同: " + (reactor.getReactorId() ==
			// newReactor.getReactorId()));
		} else {
	...
	public void returnEverLastItem(MapleCharacter chr) {
		for (MapleMapObject o : getAllItemsThreadsafe()) {
			...
			if (item.getOwner() == chr.getId()) {
				...
				broadcastMessage(InventoryPacket.removeItemFromMap(item.getObjectId(), 2, chr.getId()),
						item.getTruePosition());
				if (item.getMeso() > 0) {
	...
	private void spawnAndAddRangedMapObject(MapleMapObject mapobject, DelayedPacketCreation packetbakery) {
		...
		try {
			...
			while (itr.hasNext()) {
				chr = itr.next();
				if ((mapobject.getType() == MapleMapObjectType.MIST || chr.getTruePosition()
	...
	public void spawnAutoDrop(final int itemid, final Point pos) {
		...
		spawnAndAddRangedMapObject(mdrop,
				c -> c.announce(InventoryPacket.dropItemFromMapObject(mdrop, pos, pos, (byte) 1)));
	...
	public String spawnDebug() {
		String sb = "Mobs in map : " + this.getMobsSize() + " spawnedMonstersOnMap: " + spawnedMonstersOnMap
				+ " spawnpoints: " + monsterSpawn.size() + " maxRegularSpawn: " + maxRegularSpawn + " actual monsters: "
				+ getNumMonsters() + " monster rate: " + monsterRate + " fixed: " + fixedMob;
	...
	public void spawnItemDrop(final MapleMapObject dropper, final MapleCharacter owner, final Item item, Point pos,
		...
		spawnAndAddRangedMapObject(drop, c -> c
				.announce(InventoryPacket.dropItemFromMapObject(drop, dropper.getTruePosition(), droppos, (byte) 1)));
	...
	public void spawnMesoDrop(final int meso, final Point position, final MapleMapObject dropper,
		...
		spawnAndAddRangedMapObject(mdrop, c -> c
				.announce(InventoryPacket.dropItemFromMapObject(mdrop, dropper.getTruePosition(), droppos, (byte) 1)));
		if (!everlast) {
			mdrop.registerExpire(120000);
	 ...
	 * 刀飞炼狱推动金币
	 */
	public void spawnMesoDropEx(final int meso, final Point dropfrom, final Point dropto, final MapleMapObject dropper,
			final MapleCharacter owner, final boolean playerDrop, final byte droptype) {
		...
		spawnAndAddRangedMapObject(mdrop,
				c -> c.announce(InventoryPacket.dropItemFromMapObject(mdrop, dropfrom, droppos, (byte) 1)));
		...
		if (mist.isPoisonMist() && !mist.isMobMist()) { // 中毒类型的烟雾处理 不处理怪物的烟雾效果
			...
			poisonSchedule = tMan.register(() -> {
				for (MapleMapObject mo : getMapObjectsInRect(mist.getBox(),
					...
					if (!pvp && mist.makeChanceResult()
							&& !((MapleMonster) mo).isBuffed(MonsterStatus.MOB_STAT_Poison)) {
						if (owner != null) {
							((MapleMonster) mo).applyStatus(owner,
									new MonsterStatusEffect(MonsterStatus.MOB_STAT_Poison, 1,
											mist.getSourceSkill().getId(), null, false, mist.getSource().getDOTStack()),
									true, duration, true, mist.getSource());
						}
					}
				}
			}, 2000, 2500);
			...
			poisonSchedule = tMan.register(() -> {
				for (MapleMapObject mo : getMapObjectsInRect(mist.getBox(),
					...
					if (mist.makeChanceResult()) {
						...
						chr.addHPMP((int) (mist.getSource().getX() * (chr.getStat().getMaxHp() / 100.0)),
								(int) (mist.getSource().getX() * (chr.getStat().getMaxMp() / 100.0)));
					}
				}
			}, 2000, 2500);
	...
	public void spawnMobDrop(final Item idrop, final Point dropPos, final MapleMonster mob, final MapleCharacter chr,
			final byte droptype, final int questid) {
	...
	public void spawnMobMesoDrop(final int meso, final Point position, final MapleMapObject dropper,
			final MapleCharacter owner, final boolean playerDrop, final byte droptype) {
		final MapleMapItem mdrop = new MapleMapItem(meso, position, dropper, owner, droptype, playerDrop);

		spawnAndAddRangedMapObject(mdrop, c -> c
				.announce(InventoryPacket.dropItemFromMapObject(mdrop, dropper.getTruePosition(), position, (byte) 1)));
	...
	public void spawnMonster(final MapleMonster monster, final int spawnType, final boolean overwrite) {
		...
		if (monster.getId() == 9300166) { // 炸弹
			MapTimer.getInstance().schedule(() -> broadcastMessage(MobPacket.killMonster(monster.getObjectId(), 2)),
					3000);
		}
...
//                c.announce(MobPacket.spawnMonster(monster, spawnType, 0));
	...
	public void spawnRevives(final MapleMonster monster, final int oid) {
		...
		spawnAndAddRangedMapObject(monster, c -> {
			c.announce(MobPacket.spawnMonster(monster,
					monster.getStats().getSummonType() <= 1 ? -3 : monster.getStats().getSummonType(), oid)); // TODO
																												// effect
		});
		updateMonsterController(monster);
		spawnedMonstersOnMap.incrementAndGet();
	...
	public void updateMapObjectVisibility(MapleCharacter chr, MapleMapObject mo) {
		...
		if (!chr.isMapObjectVisible(mo)) { // 判断当前地图上的对象是否可见
			if (mo.getType() == MapleMapObjectType.MIST || mo.getType() == MapleMapObjectType.EXTRACTOR
					...
					|| mo instanceof MechDoor
					|| mo.getTruePosition().distanceSq(chr.getTruePosition()) <= mo.getRange()) {
				chr.addVisibleMapObject(mo);
				mo.sendSpawnData(chr.getClient());
			}
		} else { // 当前地图上的对象是可见的
			if (!(mo instanceof MechDoor) && mo.getType() != MapleMapObjectType.MIST
